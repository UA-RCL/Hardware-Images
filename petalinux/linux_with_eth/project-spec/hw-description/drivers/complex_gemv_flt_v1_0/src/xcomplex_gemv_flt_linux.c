...
typedef struct {
    ...
    XComplex_gemv_flt_uio_map maps[ MAX_UIO_MAPS ];
} XComplex_gemv_flt_uio_info;

/***************** Variable Definitions **************************************/
static XComplex_gemv_flt_uio_info uio_info;
...
static int uio_info_read_name(XComplex_gemv_flt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XComplex_gemv_flt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XComplex_gemv_flt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XComplex_gemv_flt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XComplex_gemv_flt_Initialize(XComplex_gemv_flt *InstancePtr, const char* InstanceName) {
	XComplex_gemv_flt_uio_info *InfoPtr = &uio_info;
